...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'name', 'email', 'password','phone','username','sponsor','ParentId','position','rank','active_status','jdate','level','tpassword','adate','PSR','TPSR','country','country_iso','dialCode'
    ];
    ...
    {
    $balance = (Auth::user()->users_incomes()+Auth::user()->tradingProfit->sum('profit')) - (Auth::user()->withdraw());
    return $balance;
    } 
 ...
 public function investMentWithWithdraw()
    ...
    {
        return  Income::where('user_id',Auth::user()->id)->where('remarks','!=','Quantify Income')->where('credit_type',0)->sum('comm');
    } 
    ...
    {
        return  Withdraw::where('user_id',Auth::user()->id)->where('status','!=','Failed')->where('walletType',1)->sum('amount');
    } 
