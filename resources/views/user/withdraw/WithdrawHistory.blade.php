    ...
    @foreach($withdraw_report as $value)
        <div data-v-01110b5f="" class="item">
            <div data-v-01110b5f="" class="left">
                <div data-v-01110b5f="" class="type">@lang('Withdrawal')</div>
            ...
            <div data-v-01110b5f="" class="right">
                ...
                <div data-v-01110b5f="" class="status {{ $value->status == 'Approved' ? 'completed' : strtolower($value->status) }}">
                    <?php echo ($value->status == "Approved") ? 'completed' : $value->status ?>
                </div>

                @if($value->status == 'Pending')
                    <form method="GET" action="{{ route('withdraw.cancel', $value->id) }}">
                     
                        <button type="submit" class="cancel-btn" style="    margin-top: 8px;">Cancel</button>
                    </form>
                @endif
            </div>
        </div>
